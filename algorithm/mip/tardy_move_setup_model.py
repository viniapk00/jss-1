...
class TardyMoveSetupModel:
    ...
    def set_objective(self):
        ...
        m.minimize(D.weight['tardy'] * m.sum(D.Up[p] * self.z[p] for p in D.P) + D.weight['move'] * m.sum(D.Emn.get((mach, n), 0.0) * q for (a, mach, n), q in self.q.items()) + D.weight['setup'] * (m.sum(D.S[a, b, mach] * y for (a, b, mach), y in self.y.items()) + m.sum(D.S0[a, mach] * depot_arc for (a, mach), depot_arc in self.d_plus.items())))
