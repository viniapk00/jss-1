...
class TardyTotalTimeModel:
    ...
    def __init__(self, data):
        ...
        self.w, self.x, self.t, self.y, self.d_plus, self.d_minus, self.q, self.C, self.z = {}, {}, {}, {}, {}, {}, {}, {}, {}
    ...
    def add_variables(self):
        ...
        for a in D.A:
            ...
            for mach in D.Ma[a]:
                ...
                self.d_plus[a, mach] = m.continuous_var(lb=0.0, ub=1.0, name=f'd_plus_{p}_{o}_{i}_{mtag}'); self.d_minus[a, mach] = m.continuous_var(lb=0.0, ub=1.0, name=f'd_minus_{p}_{o}_{i}_{mtag}')
        ...
        for a, a_prime in D.Ar:
            for mach in D.Ma[a]:
                mtag = str(mach).replace('-', '_')
                for n in D.Ma[a_prime]: self.q[a, mach, n] = m.continuous_var(lb=0.0, ub=1.0, name=f'q_{a[0]}_{a[1]}_{a[2]}_{mtag}_{str(n).replace("-", "_")}')

        # y_a,b,m in {0, 1} : 1 if operation a immediately precedes operation b on machine m
        for a, b, mach in D.immediate_candidates: self.y[a, b, mach] = m.binary_var(name=f'y_{a[0]}_{a[1]}_{a[2]}_{b[0]}_{b[1]}_{b[2]}_{str(mach).replace("-", "_")}')
    ...
    def add_constraints(self):
        ...
        for mach in D.M:
            ...
            for a in operations:
                x = self.x[a, mach]
                m.add_constraint(self.d_plus[a, mach] + m.sum(self.y[b, a, mach] for b in operations if (b, a, mach) in self.y) == x)
                m.add_constraint(m.sum(self.y[a, b, mach] for b in operations if (a, b, mach) in self.y) + self.d_minus[a, mach] == x)
            departure = m.sum(self.d_plus[a, mach] for a in operations); return_arc = m.sum(self.d_minus[a, mach] for a in operations)
        ...
        for (a, mach), depot_arc in self.d_plus.items(): m.add_constraint(self.t[a] >= D.Bm[mach] + D.S0[a, mach] - (H + D.S0[a, mach]) * (1 - depot_arc))

        # (10) Machine Time: t_b >= t_a + T_a,m + S_a,b,m - H(1 - y_a,b,m), forall (a, b, m) in immediate_candidates
        for (a, b, mach), y in self.y.items(): m.add_constraint(self.t[b] >= self.t[a] + D.Tam[a, mach] + D.S[a, b, mach] - H * (1 - y))
    ...
    def set_objective(self):
        ...
        # min W_z * (sum_{p in P} U_p * z_p) + W_t * (sum_{(a,a') in A_r, m, n} E_m,n * q_a,m,n + sum_{(a,b,m)} S_a,b,m * y_a,b,m + sum_{a in A, m in M_a} T_a,m * x_a,m + sum_{a in A, m in M_a} S_0,a,m * d^+_a,m)
        m.minimize(D.weight['tardy'] * m.sum(D.Up[p] * self.z[p] for p in D.P) + D.weight['total_time'] * (m.sum(D.Tam[a, mach] * self.x[a, mach] for a in D.A for mach in D.Ma[a]) + m.sum(D.Emn.get((mach, n), 0.0) * q for (a, mach, n), q in self.q.items()) + m.sum(D.S[a, b, mach] * y for (a, b, mach), y in self.y.items()) + m.sum(D.S0[a, mach] * depot_arc for (a, mach), depot_arc in self.d_plus.items())))
