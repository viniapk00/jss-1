...
class TardyTotalTimeModel:
    ...
    def add_constraints(self):
        ...
        for g in D.G:
            for mach in D.M:
                ...
                changeover_entries = [
                    self.y[b, a, mach]
                    for (b, a, m_idx) in self.y
                    if m_idx == mach and b[0] not in D.Pg[g] and a[0] in D.Pg[g]
                ]
                ...
                if depot_entries or changeover_entries:
                    ...
                    for a in operations:
                        m.add_constraint(entry_sum >= self.x[a, mach])
    ...
    def set_objective(self):
        ...
        processing = m.sum(
            D.Tam[a, mach] * self.x[a, mach]
            for a in D.A for mach in D.Ma[a]
        )
        movement = m.sum(
            D.Emn.get((mach, n), 0.0) * q for (a, mach, n), q in self.q.items()
        )
        setup = m.sum(D.S[a, b, mach] * y for (a, b, mach), y in self.y.items())
        setup += m.sum(
            D.S0[a, mach] * depot_arc for (a, mach), depot_arc in self.d_plus.items()
        )
        m.minimize(
