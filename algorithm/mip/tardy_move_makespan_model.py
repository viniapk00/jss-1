...
class TardyMoveMakespanModel:
    ...
    def add_constraints(self):
        ...
        for p in D.P:
            m.add_constraint(self.Cmax >= self.C[p])
        ...
        for p in D.P:
            route_bound = m.sum(D.route_lb[p, o] * self.w[p, o] for o in D.Op[p])
            m.add_constraint(self.Cmax >= route_bound)
    ...
    def set_objective(self):
        ...
        m.minimize(
            D.weight['tardy'] * tardiness
            + D.weight['move'] * movement
            + D.weight['makespan'] * self.Cmax
        )
