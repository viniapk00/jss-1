"""Weighted tardiness, movement, and makespan FJSP MIP with pairwise sequencing."""
...
class TardyMoveMakespanModel:
    objective_type = 'tardy_move_makespan'
    ...
    def __init__(self, data):
        ...
        self.w, self.x, self.t, self.q, self.y, self.C, self.z, self.Cmax = {}, {}, {}, {}, {}, {}, {}, None
    ...
    def add_variables(self):
        ...
        for a in D.A:
            ...
            for mach in D.Ma[a]: self.x[a, mach] = m.binary_var(name=f'x_{p}_{o}_{i}_{str(mach).replace("-", "_")}')
        ...
        # c_max >= 0 : Maximum completion time among all lots.
        self.Cmax = m.continuous_var(lb=0.0, ub=H, name='Cmax')
    ...
    def add_constraints(self):
        ...
        # (15) Makespan definition: c_max >= c_p, forall p in P
        for p in D.P: m.add_constraint(self.Cmax >= self.C[p])

        # (V3) Route-based tardiness bound: z_p >= sum_{o in O_p} max{0, LB_p,o - D_p} * w_p,o, forall p in P
        # (V4) Route-based makespan bound: c_max >= sum_{o in O_p} LB_p,o * w_p,o, forall p in P
        for p in D.P:
            m.add_constraint(self.Cmax >= m.sum(D.route_lb[p, o] * self.w[p, o] for o in D.Op[p]))
    ...
    def set_objective(self):
        ...
        # min W_z * (sum_{p in P} U_p * z_p) + W_e * (sum_{(a,a') in A_r, m, n} E_m,n * q_a,m,n) + W_c * c_max
        m.minimize(D.weight['tardy'] * m.sum(D.Up[p] * self.z[p] for p in D.P) + D.weight['move'] * m.sum(D.Emn.get((mach, n), 0.0) * q for (a, mach, n), q in self.q.items()) + D.weight['makespan'] * self.Cmax)
