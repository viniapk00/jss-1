...
import argparse, os, random, time, traceback
from pathlib import Path
from typing import Optional, Sequence
import numpy as np

from utils.preprocessing import ConfigLoader, DataPreprocessor, OBJECTIVES, load_objective_class
from utils.result_saver import assert_schedule_feasible, compare_results, save_heuristic_results, save_mip_result, schedule_metrics, write_run_manifest
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--objective', '--package', dest='objective', required=True, choices=list(OBJECTIVES), help='objective model to execute')
...
def _interactive_choice(value, prompt, choices):
    if value and (value in choices or value in choices.values()): return choices.get(value, value)
    while not value:
        user_input = input(prompt).strip().lower()
        value = choices.get(user_input)
        if not value: print('  Invalid choice.')
    return value
...
def _configure(args):
    ...
    if args.output: config['output_dir'] = os.path.abspath(args.output)
    if args.input_root: config['input_dir'] = os.path.abspath(args.input_root)
...
def _execute(args) -> int:
    ...
    args.dataset = _interactive_choice(args.dataset, '\nSELECT DATASET: 1=Small  2=Medium  3=Large: ', {key: key for key in DATASET_CHOICES})

    random.seed(args.seed)
    np.random.seed(args.seed)
    try: config, objective_key = _configure(args)
    except (OSError, ValueError) as exc:
        print(f'  Configuration failed - {exc}')
        return 2

    if not config or objective_key is None: return 2

    print(f"  Effective config | objective={objective_key} | solver={args.solver} | " f"mode={args.mode} | iters={config['iterations']} | seed={args.seed}")

    data = DataPreprocessor(config)
    if not data.run_pipeline(args.dataset): return 2
    ...
    if args.mode in ('mip', 'both'):
        ...
        mip_result = _run_mip(data, os.path.join(dataset_dir, 'mip'), args.solver, objective_key)
        if mip_result:
            frame, mip_objective, elapsed, mip_gap = mip_result; results['mip'] = (frame, mip_objective, elapsed)
    ...
    if args.mode in ('heuristic', 'both'):
        ...
        frame, reported_obj, order, elapsed = greedy_seed; assert_schedule_feasible(frame, data); metrics = schedule_metrics(frame, data); metrics_cache['best_greedy'] = metrics
        greedy_seed = (frame, float(metrics['objective']), order, elapsed); heuristic_results['best_greedy'] = (frame, float(metrics['objective']), elapsed)

        # Roulette Wheel
        roulette_frame, roulette_obj, _, roulette_elapsed, roulette_meta = scheduler.run_roulette(greedy_seed)
        heuristic_results['roulette'], heuristic_metas['roulette'] = (roulette_frame, roulette_obj, roulette_elapsed), roulette_meta

        # LNS
        lns_frame, lns_obj, _, lns_elapsed, lns_meta = scheduler.run_lns(greedy_seed)
        heuristic_results['lns'], heuristic_metas['lns'] = (lns_frame, lns_obj, lns_elapsed), lns_meta

        # Save heuristic results
        saved, saved_metas = save_heuristic_results(data.time, data, heuristic_results, heuristic_metas, dataset_dir, metrics_cache=metrics_cache)
        results.update(saved); metas.update(saved_metas)

    # 3. Comparison & summary table
    if results: compare_results(data.time, data, results, os.path.join(dataset_dir, 'compare'), mip_gap=mip_gap, metas=metas, metrics_cache=metrics_cache, mip_best_bound=getattr(data, 'mip_best_bound', None))

    print(f'  Output: {dataset_dir}')
    return 0


def main(argv: Optional[Sequence[str]] = None) -> int: return _execute(_parser().parse_args(argv))
