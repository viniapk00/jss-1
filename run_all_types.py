...
OBJECTIVE_TYPES = ('tardy_only', 'tardy_move', 'tardy_move_setup', 'tardy_total_time', 'tardy_move_makespan')


def _get_python_executable(custom=None, solver='cplex'):
    if custom and os.path.isfile(custom): return custom
    if solver == 'gurobi': candidates = [r"C:\Users\hkuser\AppData\Local\Programs\Python\Python312\python.exe", sys.executable, r"C:\Users\hkuser\anaconda3\python.exe", os.path.expanduser(r"~\anaconda3\python.exe"), r"C:\ProgramData\anaconda3\python.exe"]
    else: candidates = [sys.executable, r"C:\Users\hkuser\anaconda3\python.exe", os.path.expanduser(r"~\anaconda3\python.exe"), r"C:\ProgramData\anaconda3\python.exe"]
...
def main(default_solver='cplex', allow_solver_override=True):
    ...
    while not args.dataset:
        ...
        if not args.dataset: print('  Invalid choice.')

    python_exe, child_env = _get_python_executable(args.python_executable, solver=solver), os.environ.copy()
    child_env['PYTHONPATH'] = root

    dataset, stamp = DATASET_NAME[args.dataset], time.strftime('%Y%m%d_%H%M%S')
    ...
    main_file, parameter_file = os.path.join(root, 'main.py'), os.path.join(root, 'parameter.csv')
    ...
    for type_key in OBJECTIVE_TYPES:
        ...
        if code != 0 and args.stop_on_error: break
