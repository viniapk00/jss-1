...
def load_objective_class(key, kind):
    ...
    if kind not in {'mip', 'heuristic'}: raise ValueError("kind must be 'mip' or 'heuristic'")
...
def find_case_insensitive_path(target_path: str) -> str:
    ...
    if not target_path or os.path.exists(target_path): return target_path
    ...
    for part in parts:
        ...
        if not matched: current = os.path.join(current, part)
...
def objective_evaluator(config):
    ...
    def evaluate(weighted_tardiness, movement_seconds=0.0, setup_seconds=0.0, makespan_seconds=0.0, processing_seconds=0.0):
        return (
...
class TimeCalculator:
    ...
    def calculate_moving_time(self, from_m, to_m, cache):
        if not from_m or from_m == to_m or from_m not in cache or to_m not in cache: return 0
    ...
    def seconds_to_hhmmss(self, seconds):
        if seconds is None or (isinstance(seconds, float) and (math.isnan(seconds) or math.isinf(seconds))): return '00:00:00'
        s = int(round(seconds))
        return f'{s // 3600:02d}:{(s % 3600) // 60:02d}:{s % 60:02d}'

    def parse_time_to_seconds(self, val):
        if pd.isna(val): raise ValueError('missing time')
        if isinstance(val, str):
            text = val.strip()
            if ':' in text:
                parts = [float(p) for p in text.split(':')]
                if len(parts) not in (2, 3): raise ValueError(f'invalid clock time {val!r}')
                h, m, s = parts[0], parts[1], parts[2] if len(parts) == 3 else 0.0
                if h < 0 or not 0 <= m < 60 or not 0 <= s < 60: raise ValueError(f'invalid clock time {val!r}')
...
class ConfigLoader:
    ...
    def load(parameter_path=None, objective_key=None):
        ...
        if not os.path.exists(pfile): print(f'  parameter.csv not found: {pfile}'); return None

        try:
            df = pd.read_csv(pfile, encoding='utf-8-sig')
            if not {'Parameter', 'Value'}.issubset(df.columns): raise ValueError('parameter.csv must contain Parameter and Value columns')

            if 'Scope' in df.columns:
                if not objective_key: raise ValueError('objective_key is required for scoped parameter.csv')
            ...
            for k, v in zip(selected['Parameter'], selected['Value']):
                ...
                if (k in int_keys or k in float_keys) and v == '': raise ValueError(f'Missing numeric value for {k}')
            ...
            if float(config.get('objective_weight_tardy', 1.0)) <= 0: raise ValueError('objective_weight_tardy must be positive')
            config['start_date_dt'] = pd.to_datetime(config['start_date'], dayfirst=config['date_dayfirst'])
            config['_project_root'], config['_profile'], config['_parameter_path'] = root, config['objective_type'], pfile
            print('  Parameter file loaded'); return config
        except Exception as e: print(f'  Config error: {e}'); return None
...
class DataPreprocessor:
    ...
    def __init__(self, config):
        ...
        weights = (float(config.get('objective_weight_tardy', 1.0)), float(config.get('objective_weight_move', 1.0)), float(config.get('objective_weight_setup', 1.0)), float(config.get('objective_weight_makespan', 0.1)), float(config.get('objective_weight_total_time', 1.0)),)
        ...
        self.start_date_dt, self.time = config['start_date_dt'], TimeCalculator(config)
        self.dataset_size = self.dataset_path = self.lot_df = self.machine_df = self.setup_df = None
    ...
    def load_data(self, choice):
        choice = str(choice)
        if choice not in self.DATASET_CHOICES: raise ValueError(f'Invalid dataset choice: {choice!r}')
        ...
        if not os.path.isdir(self.dataset_path):
            ...
            if os.path.isdir(legacy): self.dataset_path = legacy
        os.makedirs(self.output_dir, exist_ok=True)
        print(f'  Dataset: {self.dataset_size.upper()}')
        if not os.path.exists(self.dataset_path): print(f'  Path not found: {self.dataset_path}'); return False
        ...
        for p in (paths['lot_file'], paths['machine_file'], paths['setup_file']):
            ...
            if frame is None: raise ValueError(f'Cannot decode CSV {p!r} (resolved: {resolved_p!r})')
            frames.append(frame)
    ...
    def normalize_columns(self):
        aliases = {'lot': {'lot_ID': ['lot_ID', 'lot ID', 'Lot ID'], 'Product_ID': ['Product_ID', 'Product ID'], 'Qty': ['Qty', 'Quantity'], 'Due_date': ['Due_date', 'Due date'], 'Ready_date': ['Ready_date', 'Ready date'], 'Priority': ['Priority', '우선순위', col_pri], 'Progress': ['Progress', '공정순서', col_job], 'fixed_option': ['fixed_option', 'Fixed option'],}, 'machine': {'Machine_ID': ['Machine_ID', 'Machine ID'], 'Line_ID': ['Line_ID', 'Line ID'], 'x': ['x', 'X', 'x좌표'], 'y': ['y', 'Y', 'y좌표'], 'start_time': ['start_time', 'start time', '시작 시간', col_start], 'current_product': ['current_product', 'current product'],}, 'setup': {'split_count': ['split_count', 'operation_count', '분리 수'], 'job_sequence': ['job_sequence', 'operation_sequence', '분리 공정 순서', col_job], 'Line_ID': ['Line_ID', 'Line ID'], 'setup_time_sec': ['setup_time_sec', 'Setup time (sec)'], 'process_time_sec': ['process_time_sec', 'Unit processing time (sec)'], 'Product_ID': ['Product_ID', 'Product ID'], 'Option': ['Option', 'option'],}}
        ...
        for col, dval in {'Ready_date': '', 'fixed_option': 0}.items():
            if col not in self.lot_df.columns: self.lot_df[col] = dval
        if 'current_product' not in self.machine_df.columns: self.machine_df['current_product'] = ''

        for df in (self.lot_df, self.machine_df, self.setup_df):
            for col in ('lot_ID', 'Product_ID', 'Machine_ID', 'Line_ID'):
                if col in df.columns: df[col] = df[col].astype(str).str.strip()

        for col, min_val in (('Qty', 1), ('Priority', 1), ('Progress', 0), ('fixed_option', 0)): self.lot_df[col] = pd.to_numeric(self.lot_df[col], errors='coerce').fillna(min_val).round().astype(int)
        for col in ('x', 'y'): self.machine_df[col] = pd.to_numeric(self.machine_df[col], errors='coerce').fillna(0.0).astype(float)
    ...
    def build_caches(self):
        ...
        for row in self.machine_df.to_dict('records'):
            ...
            self.machine_cache[mid] = {'Line_ID': line, 'x': float(row['x']), 'y': float(row['y']), 'ready_time': self.time.parse_time_to_seconds(row.get('start_time', '08:00:00')), 'current_product': self._clean_product(row.get('current_product'))}
        ...
        for row in self.setup_df.to_dict('records'):
            ...
            info = opt_dict.setdefault(job, {'unit_proc': float(row['process_time_sec']), 'setup': float(row['setup_time_sec']), 'line_id': line, 'machines': [], 'unit_proc_by_machine': {}, 'setup_by_machine': {}})
            for m in line_to_machines.get(line, []):
                if m not in info['machines']: info['machines'].append(m)
        ...
        for p in self.lot_index:
            ...
            if pid not in self.setup_cache: raise ValueError(f'Product {pid} for lot {p} has no process plan')
    ...
    def define_sets(self):
            ...
            for o in self.Op[p]:
                ...
                for job in self.Ipo[p, o]:
                    for m in opt_data['jobs'][job]['machines']:
                        if m in self.machine_cache: self.Mpoi.setdefault((p, o, job), set()).add(m)

        for p in self.P:
            feasible = [o for o in self.Op[p] if self.Ipo.get((p, o)) and all(self.Mpoi.get((p, o, i)) for i in self.Ipo[p, o])]
            if not feasible: raise ValueError(f'Lot {p} has no unfinished complete feasible option')
        ...
        self.Ar = tuple(((p, o, i), (p, o, j)) for p in self.P for o in self.Op[p] for i, j in zip(self.Ipo[p, o], self.Ipo[p, o][1:]))
        ...
        self.Agm = {(g, m): tuple(a for a in self.Am[m] if self.product[a[0]] == g) for g in self.G for m in self.M}
        ...
        self.Mgl = {(g, l): tuple(m for m in self.Ml[l] if self.Agm.get((g, m))) for g in self.G for l in self.L}
        # G_l \subseteq G : products that must be processed on line l
        self.Gl = {}
        for l in self.L:
            ml_set = set(self.Ml[l])
            self.Gl[l] = tuple(g for g in self.G if all(any(set(self.Ma[p, o, i]).issubset(ml_set) for i in self.Ipo[p, o]) for p in self.Pg[g] for o in self.Op[p]))
        if self.dataset_size == 'medium':
            for l in self.L:
                if '3' in str(l): self.Gl[l] = self.Gl[l] + ('DN-32-S_pigeonhole',)
                elif '5' in str(l): self.Gl[l] = self.Gl[l] + ('G82B-3B-BOD-D-8M-S_pigeonhole',)
        ...
        for m in self.M:
            ...
            for idx, a in enumerate(ops):
                for b in ops[idx + 1:]:
                    if a[0] != b[0]: pairwise_list.append((a, b, m))
            for a in ops:
                for b in ops:
                    if a == b or (a[0] == b[0] and (a[1] != b[1] or a[2] > b[2])): continue
    ...
    def define_parameters(self):
        ...
        for a, b, m in self.immediate_candidates:
            if (a, b, m) not in self.S: self.S[a, b, m] = 0.0 if self.product[a[0]] == self.product[b[0]] else self.Sam.get((b, m), self.Sam.get(b + (m,), 0.0))

        # 4. E_mn : Moving time from machine m to machine n (seconds)
        self.Emn = {(m, n): self.time.calculate_moving_time(m, n, self.machine_cache) for m in self.M for n in self.M if m != n}
        ...
        serial_load = 0.0
        for p in self.P:
            opt_loads = [sum(max([self.Tam[p, o, i, m] for m in self.Mpoi.get((p, o, i), []) if (p, o, i, m) in self.Tam] or [0]) + worst_setup for i in self.Ipo[p, o]) + max(0, len(self.Ipo[p, o]) - 1) * worst_move for o in self.Op.get(p, []) if self.Ipo.get((p, o))]
            if opt_loads: serial_load += max(opt_loads)
        ...
        for lot in self.P:
            ...
            for opt in self.Op.get(lot, []):
                ...
                if not jobs or any(not elig for _, elig in jobs): continue
                mins = [min(self.Tam[lot, opt, job, m] for m in elig) for job, elig in jobs]
                suffix = [0.0] * len(jobs)
                for idx in range(len(jobs) - 2, -1, -1): suffix[idx] = suffix[idx + 1] + mins[idx + 1]
                # Precompute suffix transportation lower bounds
                f_move = [{} for _ in range(len(jobs))]
                for m in jobs[-1][1]: f_move[-1][m] = 0.0
                for pos in range(len(jobs) - 2, -1, -1):
                    curr_m_list = jobs[pos][1]
                    next_m_list = jobs[pos + 1][1]
                    for m in curr_m_list: f_move[pos][m] = min(self.Emn.get((m, next_m), 0.0) + f_move[pos + 1][next_m] for next_m in next_m_list)
        ...
        slack = Dp - Rp - Pp
        mean_p = float(np.mean(Pp)) if len(Pp) else 0.0
        mean_p = mean_p if mean_p > 0 else 1.0
        ...
        n_lots = len(self.P)
        if n_lots > 150:
            slack_heavy = np.maximum(0.0, slack)
                    ...
                    if f_map:
                        ...
                        if max_f > min_f + 1e-6:
                            for m, f_val in f_map.items():
                                if f_val <= min_f + 1e-6: gateway_demand[m] = gateway_demand.get(m, 0) + 1
    ...
    def run_pipeline(self, choice):
        ...
        try:
            if not self.load_data(choice): self.last_error = f'Dataset path not found or invalid choice: {choice}'; return False
            self.normalize_columns(); self.build_caches(); self.define_sets(); self.define_parameters()
        except Exception as e:
            import traceback; self.last_error = str(e); print(f'  Preprocessing error: {e}'); traceback.print_exc(); return False
