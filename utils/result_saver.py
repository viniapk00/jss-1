...
from utils.preprocessing import (
    col_end, col_job, col_pri, col_start, objective_evaluator,
    seconds_per_day, tardiness_seconds,
)
...
def _nat_key(val):
    return ''.join(f'{int(p):010d}' if p.isdigit() else p.lower() for p in re.split(r'(\d+)', str(val)))
...
def validate_schedule(df, data, tol=1e-5):
    ...
    if not req.issubset(df.columns):
        return [f'missing columns: {sorted(req - set(df.columns))}']

    errs = []
    for c in (col_start, col_end, 'Setup Time', 'Moving Time', 'Processing Time'):
        v = pd.to_numeric(df[c], errors='coerce')
        if not bool(v.map(lambda x: pd.notna(x) and math.isfinite(float(x))).all()):
            errs.append(f'{c}: non-finite')
        if bool((v < -tol).any()):
            errs.append(f'{c}: negative')
    if errs:
        return errs

    if set(df['lot ID'].dropna()) != set(data.P):
        errs.append(f'lot coverage mismatch: missing={sorted(set(data.P) - set(df["lot ID"].dropna()))}')
    ...
    for lot, grp in df.groupby('lot ID', sort=False):
        ...
        if fixed and opt != fixed:
            errs.append(f'{lot}: selected option {opt} != fixed {fixed}')
        ...
        for _, r in ordered.iterrows():
            ...
            if abs(e - s - exp_p) > tol or abs(float(r['Processing Time']) - exp_p) > tol:
                errs.append(f'{lot}/{opt}/{j}: processing mismatch')
            if s + tol < float(data.Rp.get(lot, 0)):
                errs.append(f'{lot}/{opt}/{j}: starts before ready time')
            if prev is not None:
                exp_m = float(data.Emn.get((prev['Machine ID'], m), 0)) if prev['Machine ID'] != m else 0.0
                if abs(float(r.get('Moving Time', 0) or 0) - exp_m) > tol:
                    errs.append(f'{lot}/{opt}/{j}: move mismatch')
                if s + tol < float(prev[col_end]) + exp_m:
                    errs.append(f'{lot}/{opt}/{j}: precedence violation')
            prev = r
    ...
    for m, grp in df.groupby('Machine ID', sort=False):
        ...
        for _, r in ordered.iterrows():
            ...
            if abs(float(r.get('Setup Time', 0) or 0) - exp_s) > tol:
                errs.append(f'{m}/{r["lot ID"]}: setup mismatch')
            if occ + tol < float(data.Bm.get(m, 0)):
                errs.append(f'{m}/{r["lot ID"]}: occupies before machine ready')
            if prev_end is not None and occ + tol < prev_end:
                errs.append(f'{m}: overlap at {r["lot ID"]}')
...
def assert_schedule_feasible(df, data):
    ...
    if errs:
        raise RuntimeError(f'infeasible schedule: {"; ".join(errs[:5])}')
...
def validate_pairwise_schedule(df, data, tol=1e-5):
    ...
    for m, grp in df.groupby('Machine ID', sort=False):
        ...
        for i, l in enumerate(rows):
            ...
            for r in rows[i + 1:]:
                if l['lot ID'] == r['lot ID']:
                    continue
                poi2, sb, eb = (r['lot ID'], r['Option'], r[col_job]), float(r[col_start]), float(r[col_end])
                if not ((sb + tol >= ea + float(data.changeover_setup(poi1, poi2, m))) or (sa + tol >= eb + float(data.changeover_setup(poi2, poi1, m)))):
                    errs.append(f'{m}: pairwise overlap between {l["lot ID"]} and {r["lot ID"]}')
    return errs
...
def assert_pairwise_schedule_feasible(df, data):
    ...
    if errs:
        raise RuntimeError(f'infeasible pairwise schedule: {"; ".join(errs[:5])}')


def schedule_metrics(df, data):
    if df is None or df.empty:
        return {
            'objective': 0.0, 'objective_per_lot': 0.0, 'weighted_tardiness': 0.0, 'weighted_tardiness_days': 0.0,
            'moving_seconds': 0.0, 'moving_days': 0.0, 'setup_seconds': 0.0, 'setup_days': 0.0,
            'transition_seconds': 0.0, 'transition_days': 0.0, 'processing_seconds': 0.0, 'processing_days': 0.0,
            'total_time_seconds': 0.0, 'total_time_days': 0.0, 'total_time_component': 0.0,
            'average_flowtime_seconds': 0.0, 'average_flowtime_days': 0.0, 'tardiness_seconds': 0.0,
            'tardy_lots': 0, 'tardy_lot_pct': 0.0, 'makespan_seconds': 0.0, 'makespan_days': 0.0,
            'makespan_component': 0.0, 'schedule_span_seconds': 0.0, 'product_machine_pairs': 0,
            'lot_items': [], 'lot_details': []
        }
    ...
    for lot_id, grp in df.groupby('lot ID'):
        ...
        lot_details.append({
            'lot_ID': lot_id, 'product': last.get('Product ID', ''), 'option': last.get('Option', ''),
            'priority': last.get(col_pri, ''), 'priority_weight': weight, 'due_date': str(last.get('Due date', '')),
            'completion_sec': comp, 'delay_sec': delay, 'flowtime_sec': flow, 'weighted_penalty': pen,
            'total_move_sec': m_s, 'total_setup_sec': s_s, 'makespan_sec': cmax if lot_id == crit_lot else 0.0,
            'total_processing_sec': p_s, 'total_time_sec': p_s + m_s + s_s,
            'lot_objective': evaluate_obj(pen, m_s, s_s, cmax if lot_id == crit_lot else 0.0, p_s),
            'is_tardy': delay > TARDY_EPS
        })
    ...
    return {
        'objective': obj, 'objective_per_lot': obj / n_lots if n_lots else 0.0,
        'weighted_tardiness': w_tardy, 'weighted_tardiness_days': w_tardy,
        'moving_seconds': m_tot, 'moving_days': m_tot,
        'setup_seconds': s_tot, 'setup_days': s_tot,
        'transition_seconds': m_tot + s_tot, 'transition_days': m_tot + s_tot,
        'processing_seconds': p_tot, 'processing_days': p_tot,
        'total_time_seconds': tot_time, 'total_time_days': tot_time,
        'total_time_component': float(data.config.get('objective_weight_total_time', 1.0)) * tot_time if data.config.get('objective_type') == 'tardy_total_time' else 0.0,
        'average_flowtime_seconds': flow_sum / max(1, n_lots), 'average_flowtime_days': flow_sum / max(1, n_lots),
        'tardiness_seconds': tardy_sec, 'tardy_lots': n_tardy, 'tardy_lot_pct': 100.0 * n_tardy / n_lots if n_lots else 0.0,
        'makespan_seconds': cmax, 'makespan_days': cmax, 'makespan_component': evaluate_obj(0.0, 0.0, 0.0, cmax),
        'schedule_span_seconds': cmax - float(df[col_start].min()), 'schedule_span_days': cmax - float(df[col_start].min()),
        'product_machine_pairs': int(df[['Machine ID', 'Product ID']].drop_duplicates().shape[0]),
        'critical_lot': crit_lot, 'lot_items': lot_items, 'lot_details': lot_details
    }
...
def format_objective_breakdown(obj, metrics, objective_type=''):
    ...
    return (
        f"{pfx}: {_fmt(obj, 15)} "
        f"(delay={_fmt(g('weighted_tardiness_days', 'weighted_tardiness'), 6)}, "
        f"processing={_fmt(g('processing_days', 'processing_seconds'), 6)}, "
        f"total_time={_fmt(g('total_time_days', 'total_time_seconds'), 6)}, "
        f"total_time_term={_fmt(g('total_time_component'), 6)}, "
        f"avg_flow={_fmt(g('average_flowtime_days', 'average_flowtime_seconds'), 6)}, "
        f"makespan={_fmt(g('makespan_days', 'makespan_seconds'), 6)}, "
        f"makespan_term={_fmt(g('makespan_component'), 6)}, "
        f"move={_fmt(g('moving_days', 'moving_seconds'), 6)}, "
        f"setup={_fmt(g('setup_days', 'setup_seconds'), 6)}, "
        f"transition={_fmt(g('transition_days', 'transition_seconds'), 6)}, "
        f"tardy={metrics.get('tardy_lots', 0)}/{len(metrics.get('lot_items', []))})"
    )
...
def save_result(tc, results_dir, ds_size, data, schedule_df, solve_time, method='heuristic'):
    if schedule_df is None or schedule_df.empty:
        return None
    assert_schedule_feasible(schedule_df, data)
    metrics = schedule_metrics(schedule_df, data)
    ...
    if tc is not None:
        ...
        for c in (col_start, col_end):
            out[c] = (tc.start_dt + pd.to_timedelta(out[c].fillna(0), unit='s')).dt.strftime('%m/%d/%Y %H:%M:%S')
    ...
    summary = {
        'method': method, **{k: metrics[k] for k in ('objective', 'objective_per_lot', 'weighted_tardiness', 'moving_seconds', 'setup_seconds', 'product_machine_pairs', 'transition_seconds', 'processing_seconds', 'total_time_seconds', 'average_flowtime_seconds', 'total_time_component', 'makespan_component', 'tardy_lots', 'tardy_lot_pct', 'makespan_seconds', 'schedule_span_seconds')},
        'solve_time': solve_time, 'lots': schedule_df['lot ID'].nunique(), 'jobs': len(schedule_df), 'dataset': ds_size,
        'objective_type': data.config.get('objective_type', ''), 'start_date': tc.start_dt.strftime('%m/%d/%Y') if tc is not None else ''
    }
...
def write_run_manifest(output_dir, config, data, seed, mode, solver):
    ...
    manifest = {
        ...
        'seed': int(seed), 'mode': mode, 'solver': solver,
        'configuration': {k: v.isoformat() if hasattr(v, 'isoformat') else v for k, v in sorted(config.items())},
        'environment': {'python': sys.version, 'executable': sys.executable, 'platform': platform.platform(), 'logical_cpu_count': os.cpu_count()},
        'loaded_parameter': {'path': p_path, 'sha256': sha256_file(p_path) if os.path.isfile(p_path) else None},
        'code_sha256': code_hashes,
    }
    path = os.path.join(output_dir, 'run_manifest.json')
    with open(path, 'w', encoding='utf-8') as h:
        json.dump(manifest, h, indent=2, ensure_ascii=False, default=str)
    return path
...
def extract_mip_schedule(mip):
    ...
    for (poi, m), var in mip.x.items():
        if var.solution_value > 0.5:
            ...
            j = {
                'lot': lot, 'product': row['Product_ID'], 'option': opt, 'job': op, 'machine': m,
                'start': s, 'finish': s + proc, 'proc': proc, 'setup': 0.0, 'move': 0.0,
                'prev_machine': '', 'qty': row['Qty'], 'due': row['Due_date'], 'pri': row['Priority']
            }
            jobs.append(j)
            by_key[lot, opt, op, m] = j
    ...
    for lot, opt in selected.items():
        ...
        for op1, op2 in zip(seq, seq[1:]):
            k1 = next((k for k in by_key if k[:3] == (lot, opt, op1)), None)
            k2 = next((k for k in by_key if k[:3] == (lot, opt, op2)), None)
    ...
    if data.setup_mode == 'immediate':
        ...
        for poi1, poi2, m in selected_arcs:
            k2 = poi2 + (m,)
            if k2 in by_key:
                by_key[k2]['setup'] = data.changeover_setup(poi1, poi2, m)
        for k, j in by_key.items():
            if (k in depot_starts) or (not depot_starts and k not in successors and data.initial_product.get(k[3]) is not None):
                j['setup'] = data.changeover_setup(None, k[:3], k[3])
    else:
        for m in data.M:
            m_jobs = sorted([k for k in by_key if k[3] == m], key=lambda k: by_key[k]['start'])
            if m_jobs:
                by_key[m_jobs[0]]['setup'] = data.changeover_setup(None, m_jobs[0][:3], m)
                for k1, k2 in zip(m_jobs, m_jobs[1:]):
                    by_key[k2]['setup'] = data.changeover_setup(k1[:3], k2[:3], m)

    schedules = {m: [] for m in data.M}
    for k, j in by_key.items():
        schedules[j['machine']].append(j)
    for m in schedules:
        schedules[m].sort(key=lambda j: (j['start'], str(j['lot']), j['job']))
    return schedules


def _make_summary_row(method, solver_name, run_status, obj, metrics, mip_time, total_time, gap_pct, data, solver_obj=None, proven=False, claim='', model=None, best_bound=None, nodes=None, status=None):
    return {
        'method': method, 'solver': solver_name, 'run_status': run_status,
        'objective': obj, 'objective_per_lot': metrics.get('objective_per_lot', 0.0),
        'weighted_tardiness': metrics.get('weighted_tardiness', metrics.get('weighted_tardiness_days', 0.0)),
        'moving_seconds': metrics.get('moving_seconds', metrics.get('moving_days', 0.0)),
        'setup_seconds': metrics.get('setup_seconds', metrics.get('setup_days', 0.0)),
        'makespan_component': metrics.get('makespan_component', 0.0),
        'processing_seconds': metrics.get('processing_seconds', metrics.get('processing_days', 0.0)),
        'total_time_seconds': metrics.get('total_time_seconds', metrics.get('total_time_days', 0.0)),
        'total_time_component': metrics.get('total_time_component', 0.0),
        'average_flowtime_seconds': metrics.get('average_flowtime_seconds', metrics.get('average_flowtime_days', 0.0)),
        'product_machine_pairs': metrics.get('product_machine_pairs', 0),
        'transition_seconds': metrics.get('transition_seconds', metrics.get('transition_days', 0.0)),
        'tardy_lots': metrics.get('tardy_lots', 0), 'tardy_lot_pct': metrics.get('tardy_lot_pct', 0.0),
        'solver_objective': solver_obj, 'objective_delta': (obj - solver_obj) if solver_obj is not None else 0.0,
        'solver_time': mip_time, 'solve_time': mip_time, 'total_pipeline_time': total_time,
        'gap_pct': gap_pct, 'solver_gap_pct': gap_pct, 'reported_schedule_gap_pct': gap_pct,
        'normalized_incumbent': (run_status == 'solved_normalized'), 'proven_optimal': proven,
        'optimality_claim': claim, 'lots': len(data.P), 'machines': len(data.M),
        'variables': getattr(model, 'number_of_variables', None) if model else None,
        'constraints': getattr(model, 'number_of_constraints', None) if model else None,
        'best_bound': best_bound, 'nodes': nodes, 'solver_status': status,
    }
...
def normalize_mip_schedule(df, data):
    ...
    for _ in range(30):
        ...
        for m, grp in df.groupby('Machine ID', sort=False):
            ordered = grp.assign(
                _occ=pd.to_numeric(grp[col_start], errors='coerce').fillna(0) - pd.to_numeric(grp['Setup Time'], errors='coerce').fillna(0)
            ).sort_values(['_occ', col_end], kind='mergesort')
            prev_end = None
            for idx in ordered.index:
                ...
                if prev_end is not None:
                    min_start = max(min_start, prev_end + setup)
                if min_start > curr_s + 1e-5:
                    ...
                    prev_end = max(prev_end or float('-inf'), float(df.at[idx, col_end]))

        for lot, grp in df.groupby('lot ID', sort=False):
            ordered = grp.assign(
                _j=pd.to_numeric(grp[col_job], errors='coerce')
            ).sort_values(['_j', col_job], kind='mergesort')
            prev = None
...
def save_mip_result(tc, data, mip, out_dir, solver_name='cplex'):
    ...
    if getattr(data, 'setup_mode', None) == 'pairwise':
        assert_pairwise_schedule_feasible(gdf, data)
    ...
    if normalized:
        print(f'  NOTE: normalized solver incumbent {solver_obj:.9f} to validated schedule {obj:.9f}')
    ...
    row = _make_summary_row(
        'mip', solver_name, 'solved_normalized' if normalized else 'solved', obj, metrics, mip.solve_time, total_time,
        gap_pct, data, solver_obj=solver_obj, proven=proven, claim='solver_tolerance_optimal' if proven else 'not_proven',
        model=mip.model, best_bound=best_bound, nodes=getattr(mip, 'node_count', None), status=status
    )
...
def comparison_summary(tc, data, results, cmp_dir, mip_gap=None, metas=None, metrics_cache=None, mip_failure=None, mip_best_bound=None):
    ...
    if mip_failure and 'mip' not in results:
        results = {'mip': (pd.DataFrame(), None, mip_failure.get('elapsed')), **results}
    ...
    for name, (df, _, elapsed) in results.items():
        ...
        stats[name] = {
            'Lots': df['lot ID'].nunique(), 'Products': df['Product ID'].nunique(),
            'Setup Time': tc.seconds_to_hhmmss(df['Setup Time'].sum()) if tc is not None else '00:00:00',
            'Setup Count': int((df['Setup Time'] > 0).sum()),
            'Product-Machine Pairs': m['product_machine_pairs'],
            'Moving Time': tc.seconds_to_hhmmss(m['moving_seconds']) if tc is not None else '00:00:00',
            'Moving Count': int((df['Moving Time'] > 0).sum()),
            'Proc Time': tc.seconds_to_hhmmss(df['Processing Time'].sum()) if tc is not None else '00:00:00',
            'Processing (sec)': f"{m['processing_seconds']:.2f}", 'Total Time (sec)': f"{m['total_time_seconds']:.2f}",
            'Average Flow Time (sec)': f"{m['average_flowtime_seconds']:.2f}",
            'Makespan': tc.seconds_to_hhmmss(m['makespan_seconds']) if tc is not None else '00:00:00',
            'Schedule Span': tc.seconds_to_hhmmss(m['schedule_span_seconds']) if tc is not None else '00:00:00',
            'Tardiness': tc.seconds_to_hhmmss(m['tardiness_seconds']) if tc is not None else '00:00:00',
            'Tardy Lots': m['tardy_lots'],
        }

    meta_val = lambda n, k: str(metas.get(n, {}).get(k, '-'))
    ctx = {
        'stats': stats, 'objs': {n: exact_objs.get(n, 'N/A') for n in results},
        'times': {n: (f'{el:.2f}s' if el is not None else 'N/A') for n, (_, _, el) in results.items()},
        'iters': {n: meta_val(n, 'number_iterations') for n in results},
        'best_iter': {n: meta_val(n, 'best_iteration') for n in results},
        'updates': {n: meta_val(n, 'number_updates') for n in results},
    }
    ...
    if bound is None and 'mip' in exact_objs:
        if mip_gap is not None and abs(mip_gap) <= 1e-6:
            bound = exact_objs['mip']
        elif mip_gap is not None and mip_gap < 100.0:
            bound = exact_objs['mip'] * (1.0 - mip_gap / 100.0)
    ...
    if keys:
        ...
        for m in metrics_rows:
            print(f'{m:<28}' + ''.join(f" {str(ctx['stats'][n].get(m, 'N/A')):>{col_w}}" for n in keys))
        print('-' * width)
        ...
        if has_gap_info:
            print(f"{'MIP Gap':<28}" + ''.join(f" {gap_values.get(n, '-'):>{col_w}}" for n in keys))
        print(sep)
    ...
    if has_gap_info:
        rows.append({'Metric': 'MIP Gap', **{labels.get(n, n): gap_values.get(n, '-') for n in keys}})
