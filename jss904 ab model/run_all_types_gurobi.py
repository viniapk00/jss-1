...
def _get_python_executable(custom=None):
    ...
    for c in candidates:
        if c and os.path.isfile(c):
            try:
                res = subprocess.run([c, '-c', 'import numpy, pandas, gurobipy'], capture_output=True, timeout=5)
                if res.returncode == 0:
                    return c
            except Exception:
                pass
...
def _build_parser():
    parser = argparse.ArgumentParser(description='Run every objective type through Gurobi optimizer.')
...
def main():
    ...
    python_exe = _get_python_executable(args.python_executable)
    child_env = os.environ.copy()
    child_env['PYTHONPATH'] = root

    dataset = DATASET_NAME[args.dataset]
    stamp = time.strftime('%Y%m%d_%H%M%S')
    out_root = os.path.abspath(args.out_root or os.path.join(root, 'output', 'all_types_gurobi', f'{dataset}_{args.mode}_{stamp}'))
    ...
    for type_key in OBJECTIVE_TYPES:
        ...
        print(f"\n{'=' * 80}\nRUN TYPE: {type_key} | dataset={dataset} | mode={args.mode} | solver=GUROBI\n{'=' * 80}", flush=True)
    ...
    print(f"\n{'=' * 80}\nALL TYPES GUROBI RUN COMPLETE: {len(runs)} objectives executed\nOutput: {out_root}\n{'=' * 80}\n")
