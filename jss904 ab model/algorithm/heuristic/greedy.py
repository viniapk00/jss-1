...
class BaseScheduler:
    ...
    def lot_order(self, weights=None, family_batch=None):
        ...
        if weights is None:
            combined_scores = state.priority_scores + state.due_scores
        else:
            priority_weight = weights.get('priority', weights.get('p', 0.5))
            due_date_weight = weights.get('due_date', weights.get('d', 0.5))
        ...
        if not use_clustering:
            sorted_indices = (-combined_scores).argsort(kind='mergesort')
            return [data.P[idx] for idx in sorted_indices]

        # Step 2: Global Product Clustering: Group lots by Product ID
        product_to_lots = {}
        for idx, lot in enumerate(data.P):
            product_id = data.product.get(lot, '')
            product_to_lots.setdefault(product_id, []).append((idx, lot))

        # Step 3: Compute product family urgency tuple to rank clusters
        product_urgency = {}
        horizon = 172800 if weights is None else weights.get('horizon', 172800)
        for product_id, items in product_to_lots.items():
            minimum_due_date = min(data.Dp.get(lot, 0) for _, lot in items)
            maximum_score = max(combined_scores[idx] for idx, _ in items)
        ...
        for product_id in sorted_products:
            items = product_to_lots[product_id]
            sorted_items = sorted(items, key=lambda item: (-combined_scores[item[0]], data.Dp.get(item[1], 0)))
            ordered_lots.extend(lot for _, lot in sorted_items)
    ...
    def option_mapping(self, lot, availability, last_operation, context):
        ...
        data = self.data
        state = self.state
        fixed_option = data.fp.get(lot, 0)

        # Enforce fixed route option if specified by input parameter
        if fixed_option > 0 and (lot, fixed_option) not in state.option_meta:
            raise ValueError(f'Lot {lot} fixed_option={fixed_option} is infeasible')

        if fixed_option > 0:
            available_options = [fixed_option]
        else:
            available_options = [opt for opt in data.Op.get(lot, []) if (lot, opt) in state.option_meta]

        if not available_options:
            return float('inf'), None, None

        due_date = data.Dp.get(lot, float('inf'))
        priority = data.Up.get(lot, 1.0)
        best_candidate = None

        # Evaluate candidate options and select the one yielding minimal composite objective
        for option in available_options:
            completion_time, operations, moving_time, setup_time, processing_time = self.machine_assign(
                lot, option, availability, last_operation, priority, due_date, context
            )
            tardiness_sec = tardiness_seconds(completion_time, due_date)
            objective_score = state.objective(
                context['weighted_tardiness'] + priority * tardiness_sec,
                context['movement_seconds'] + moving_time,
                context['setup_seconds'] + setup_time,
                max(context['makespan_seconds'], completion_time),
                context['processing_seconds'] + processing_time,
            )
    ...
    def machine_assign(self, lot, option, availability, last_operation, priority, due_date, context):
        ...
        current_product = data.product.get(lot, '')

        touched_machine_list = list(touched_machines)
        local_availability = availability[touched_machine_list]
        local_predecessor = last_operation[touched_machine_list]
        ...
        for job_position, (job_sequence, eligible_machines) in enumerate(jobs):
            operation_key = (lot, option, job_sequence)
            remaining_suffix_time = suffix_remaining[job_position]
            future_move_map = future_move_maps[job_position]
            candidate_machines = tuple(
                (machine, local_machine_index[state.machine_index[machine]], data.Tam[operation_key + (machine,)])
                for machine in eligible_machines
            )
            expanded_routes = []

            for (current_end_time, previous_machine, total_moving_time, total_setup_time,
                 total_processing_time, machine_path, current_availability,
                 current_predecessor, current_operations) in routes:
                ...
                for machine, local_index, process_duration in candidate_machines:
                    ...
                    next_availability = current_availability.copy()
                    next_predecessor = current_predecessor.copy()
                    next_availability[local_index] = end_time
                    next_predecessor[local_index] = operation_key
                    ...
                    estimated_objective = state.objective(
                        context['weighted_tardiness'] + priority * tardiness_sec,
                        context['movement_seconds'] + total_moving_time + transport_duration + expected_future_move,
                        context['setup_seconds'] + total_setup_time + setup_duration,
                        max(context['makespan_seconds'], end_time + remaining_suffix_time + expected_future_move),
                        context['processing_seconds'] + total_processing_time + process_duration,
                    )

                    # Setup continuity tie-breaker: prefer continuing the same product (zero changeover)
                    same_product = False
                    if preceding_operation:
                        pred_lot = preceding_operation[0]
                        same_product = (data.product.get(pred_lot) == current_product)

                    # Gateway reservation penalty: reserve multi-operation transit hubs for multi-op jobs
                    gateway_demand = getattr(state, 'gateway_demand', {})
                    res_penalty = gateway_demand.get(machine, 0) if expected_future_move == 0 else 0

                    # Lexicographic sorting key for beam selection
                    sorting_key = (
                        round(estimated_objective, 4),
                        not same_product,            # 1. Prefer machines currently set up for the same product
                    ...
                    expanded_routes.append((
                        sorting_key, end_time, machine,
                        total_moving_time + transport_duration,
                        total_setup_time + setup_duration,
                        total_processing_time + process_duration,
                        next_path, next_availability, next_predecessor, next_operations
                    ))

            # Prune expanded candidates to top beam_limit paths
            expanded_routes.sort(key=lambda route_item: route_item[0])
            beam_limit = max(64, getattr(state, 'route_limit', 64))
            routes = [route_item[1:] for route_item in expanded_routes[:beam_limit]]

        # Select the winning path from the beam search
        (final_end_time, _discarded_machine, final_move_time, final_setup_time,
         final_proc_time, _discarded_path, _discarded_availability,
         _discarded_predecessor, scheduled_operations) = routes[0]
    ...
    def schedule(self, lot_order, output=False):
        ...
        if (output or len(order) == len(data.P)) and (len(order) != len(data.P) or set(order) != set(data.P)):
            raise ValueError('lot_order must be a complete permutation of data.P')

        # Initialize machine availability with B_m (machine ready times)
        availability = state.initial_availability.copy()
        last_operation = np.full(len(state.machines), None, dtype=object)
        ...
        for lot in order:
            ...
            if chosen_option is None:
                raise RuntimeError(f'Greedy decoder found no feasible option for lot {lot}')

            lot_row = data.lot_index[lot]
            ...
            for machine, begin_time, end_time, setup_duration, transport_duration, process_duration, job_sequence in operations:
                ...
                if output:
                    schedule_rows.append({
                        'lot ID': lot,
                        'Product ID': lot_row['Product_ID'],
                        'Option': chosen_option,
                        'Qty': lot_row['Qty'],
                        'Due date': lot_row['Due_date'],
                        col_pri: lot_row['Priority'],
                        col_job: job_sequence,
                        'Machine ID': machine,
                        'Prev Machine': previous_machine,
                        col_start: begin_time,
                        col_end: end_time,
                        'Setup Time': setup_duration,
                        'Moving Time': transport_duration,
                        'Processing Time': process_duration,
                    })
                availability[machine_index] = end_time
                last_operation[machine_index] = (lot, chosen_option, job_sequence)
                previous_machine = machine

            lot_completion_time = operations[-1][2]
            lot_due_date = data.Dp.get(lot, 0)
        ...
        total_objective = state.objective(
            context['weighted_tardiness'],
            context['movement_seconds'],
            context['setup_seconds'],
            context['makespan_seconds'],
            context['processing_seconds'],
        )

        if output:
            return pd.DataFrame(schedule_rows), total_objective, (availability, last_operation)
