...
class DataPreprocessor:
    ...
    def __init__(self, config):
        ...
        self.Ma = self.Mpoi           # Alias for compatibility
    ...
    def define_sets(self):
        ...
        self.Mgl = {
            (g, l): tuple(
                m for m in self.Ml[l]
                if any(m in self.Mpoi.get((p, o, i), ()) for p in self.Pg[g] for o in self.Op[p] for i in self.Ipo[p, o])
            )
        ...
        for l in self.L:
            ...
            self.Gl[l] = tuple(
                g for g in self.G
                if all(
                    any(set(self.Mpoi[p, o, i]).issubset(ml_set) for i in self.Ipo[p, o])
        ...
        # Operation rank lookup for deterministic ordering of (p, o, i)
        self.operation_rank = {
            (p, o, i): idx
            for idx, (p, o, i) in enumerate((p, o, i) for p in self.P for o in self.Op[p] for i in self.Ipo[p, o])
        }
        ...
        for m in self.M:
            ops = [
                ...
                for p in self.P
                for o in self.Op[p]
                for i in self.Ipo[p, o]
                if m in self.Mpoi.get((p, o, i), ())
            ]
            for idx, poi1 in enumerate(ops):
                for poi2 in ops[idx + 1:]:
                    if poi1[0] != poi2[0]:
                        pairwise_list.append((poi1, poi2, m))
            for poi1 in ops:
                for poi2 in ops:
                    if poi1 == poi2 or (poi1[0] == poi2[0] and (poi1[1] != poi2[1] or poi1[2] > poi2[2])):
                        continue
                    immediate_list.append((poi1, poi2, m))
    ...
    def changeover_setup(self, poi1, poi2, m):
        """Lookup setup time from precomputed parameter dictionary S or S0."""
        return self.S0.get((poi2, m) if len(poi2) == 3 else poi2 + (m,), 0.0) if poi1 is None else self.S.get((poi1, poi2, m), 0.0)
    ...
    def define_parameters(self):
        ...
        # 3. T_poim, S_poim, S_0poim : Processing time, base setup time, and initial setup from depot
        for p in self.P:
            pid = self.lot_index[p]['Product_ID']
            ...
            for o in self.Op[p]:
                for job in self.Ipo[p, o]:
                    ...
                    for m in self.Mpoi[poi]:
                        ...
                        self.Tpoim[poi, m] = proc_val
                        self.Tpoim[p, o, job, m] = proc_val
                        self.Spoim[poi, m] = setup_val
                        self.Spoim[p, o, job, m] = setup_val
                        self.S0poim[poi, m] = s0_val
                        self.S0poim[p, o, job, m] = s0_val

        # S_(poi1, poi2, m) and S_0_(poi, m) : setup parameter dictionaries (Slide 1)
        self.S0 = {}
        for p in self.P:
            for o in self.Op[p]:
                for i in self.Ipo[p, o]:
                    poi = (p, o, i)
                    for m in self.Mpoi[poi]:
                        val = self.S0am[p, o, i, m]
                        self.S0[poi, m] = val
                        self.S0[p, o, i, m] = val

        self.S = {}
        for poi1, poi2, m in self.pairwise_candidates:
            s_val = 0.0 if self.product[poi1[0]] == self.product[poi2[0]] else self.Sam.get(poi2 + (m,), 0.0)
            self.S[poi1, poi2, m] = s_val
            s_rev = 0.0 if self.product[poi1[0]] == self.product[poi2[0]] else self.Sam.get(poi1 + (m,), 0.0)
            self.S[poi2, poi1, m] = s_rev
        for poi1, poi2, m in self.immediate_candidates:
            if (poi1, poi2, m) not in self.S:
                self.S[poi1, poi2, m] = 0.0 if self.product[poi1[0]] == self.product[poi2[0]] else self.Sam.get(poi2 + (m,), 0.0)
        ...
        # Light/Medium traffic: exponential slack dominates (exact matching on benchmark instances)
        # Heavy traffic (lots > 150): WSPT density-enhanced ATC clears bottleneck queues
        n_lots = len(self.P)
        k_exp = 5.0 if n_lots > 150 else 2.0
        p_pow = 1.2 if n_lots > 150 else 0.5
        urgency = (Up / np.power(np.maximum(Pp, 1.0), p_pow)) * np.exp(-np.maximum(0.0, slack) / (k_exp * mean_p))
        ...
        # Precompute diverse global dispatching orders
